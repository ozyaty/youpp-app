...
const ActivityPage = () => {
  ...
  return (
    <div>
      <Header title="Activity" showBackButton />
      <motion.div
        className="p-4"
        ...
        {activities.length > 0 ? (
          <div className="space-y-3">
            {activities.map((activity) => {
              ...
              return(
                <div key={activity.id} className={`p-3 rounded-lg flex items-start space-x-3 ${activity.alert ? 'bg-red-500/10 border border-red-500/30' : 'bg-telegram-secondary-bg border border-telegram-divider'}`}>
                  <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${activity.alert ? 'bg-red-500/20' : 'bg-telegram-button-color/20'}`}>
                    <Icon size={18} className={activity.alert ? 'text-red-500' : 'text-telegram-button-color'} />
                  </div>
                  <div>
                    <p className={`text-sm font-medium ${activity.alert ? 'text-red-700 dark:text-red-400' : 'text-telegram-text'}`}>{activity.description}</p>
                    <p className={`text-xs ${activity.alert ? 'text-red-600 dark:text-red-500' : 'text-telegram-hint'}`}>{activity.time}</p>
                  </div>
                </div>
              )}
            )}
